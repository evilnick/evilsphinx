...
workspace "Canonical K8s Workspace" {
    model {
        ...
        user = person "K8s User" "Interacts with the workloads hosted in K8s"
       ...
       k8s_snap = softwareSystem "K8s Snap Distribution" "The Kubernetes distribution in a snap" {
            ...
            kubernetes = container "Kubernetes Services" "API server, kubelet, kube-proxy, scheduler, kube-controller" {
                systemd = component "systemd daemons" "Daemons running the k8s services" 
            ...
            components = container "Components" "Core components for the k8s distribution" {
                ...
                rbac = component "RBAC" "User authorisation"
                loadbalancer = component "Load-balancer" "The load balancer (from Cilium)"
            }

            k8sd = container "K8sd" "Daemon implementing the functionality available in the k8s snap" {
                cli = component "CLI" "The CLI offered" "CLI"
        ...
        admin -> cli "Sets up and configures the cluster"
        ...
        k8s_snap -> iam "User's identity is retrieved"
